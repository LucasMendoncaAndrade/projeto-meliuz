...
def get_5_recommendations(data_path):
    ...
    products_df_grouped = products_df.groupby('daily_income_class', observed=False).apply(lambda x: x.sample(1), include_groups=False)
    products = products_df_grouped.reset_index(drop=True)[['product_title', 'product_price', 'product_image_url', 'store_name']]
